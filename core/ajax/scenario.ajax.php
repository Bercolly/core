...
try {
	...
	if (init('action') == 'loadTemplateDiff') {
		$path = __DIR__ . '/../../data/scenario';
		if (!file_exists($path . '/' . init('template'))) {
			throw new Exception( __('Fichier non trouvé : ' . $path . '/' . init('template'), __FILE__));
		}
		$return = array();
	...
	if (init('action') == 'templateupload') {
		...
		if (!in_array($extension, array('.json'))) {
			throw new Exception(__('Extension du fichier non valide (autorisé .json) : ' . $extension, __FILE__);
		}
